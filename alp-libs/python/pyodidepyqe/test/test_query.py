...
def test_raise_value_error_when_criteria_group_with_none_match_criteria_is_added(setup, monkeypatch):
    ...
    with pytest.raises(ValueError):
        query.add_criteria_group(CriteriaGroup(None, [Person.Patient()]))
...
def test_clear_study_config(setup, monkeypatch):
    # Given
    monkeypatch.setenv('PYQE_STUDY_CONFIG_ID', 'selected.pa.config.id')
    monkeypatch.setenv('PYQE_STUDY_CONFIG_VERSION', 'A')
    monkeypatch.setenv('PYQE_STUDY_CONFIG_ASSIGNED_NAME', 'selected.assignment.name')
    monkeypatch.setattr('sys.stdin', StringIO('1'))
    query = Query('query')
...
def test_configure_columns_using_config_with_interactions(setup, monkeypatch):
    # Given
    monkeypatch.setattr('sys.stdin', StringIO('1'))
    monkeypatch.setenv('PYQE_STUDY_CONFIG_ID', 'config.id.with.interactions')
    monkeypatch.setenv('PYQE_STUDY_CONFIG_VERSION', 'A')
    monkeypatch.setenv('PYQE_STUDY_CONFIG_ASSIGNED_NAME', 'assignment.name')
    ...
    # Then
    assert query._column_config_paths == ['patient.attributes.age', 'patient.attributes.gender',
                                          'patient.interactions.visit.attributes.visittype',
    ...
    assert query._filter_attributes_config_paths_dict == {
        ...
        'Visit': {
            'Visit Type': 'patient.interactions.visit.attributes.visittype',
            'Visit Date': 'patient.interactions.visit.attributes.visitdate',
        },
        'ConditionEra': {
            'Condition Era Id': 'patient.interactions.conditionera.attributes.conditioneraid'
        }
    }
...
def test_generate_cohort_columns(setup, monkeypatch):
    ...
    assert cohort_columns == [
        ...
        {'configPath': 'patient.interactions.visit.attributes.visittype', 'order': '', 'seq': 2},
        {'configPath': 'patient.interactions.visit.attributes.visitdate', 'order': '', 'seq': 3},
        {'configPath': 'patient.interactions.conditionera.attributes.conditioneraid', 'order': '', 'seq': 4}]
    ...
    assert cohort == {
        ...
        'cohortDefinition': {
            ...
            'columns': [
                ...
                {
                    'configPath': 'patient.interactions.visit.attributes.visittype',
                    'order': '',
                    'seq': 2
                },
                {
...
def _get_mock_frontend_config(pa_config, config_id, config_version):
    fe_config = {
        'config': {
            'patient': {
                'attributes': {
    ...
    if config_id == 'config.id.with.interactions':
        fe_config['config']['patient']['interactions'] = {}
        fe_config['config']['patient']['interactions']['visit'] = {
            'name': 'Visit',
            'attributes': {
                'visittype': {
                    'name': 'Visit Type'
        ...
        fe_config['config']['patient']['interactions']['conditionera'] = {
            'name': 'Condition Era',
            'attributes': {
                'conditioneraid': {
