...
import { StrategusDrawer } from "./StrategusDrawer";
import "./StrategusNode.scss";

export interface StrategusNodeData extends NodeDataState {
  code: string;
}

export const StrategusNode = (node: NodeProps<StrategusNodeData>) => {
  const { data } = node;
  ...
  return (
    <>
      <NodeLayout<StrategusNodeData>
        className="strategus-node"
        name={data.name}
      ...
      <StrategusDrawer
        node={node}
        title="Configure Strategus"
        className="strategus-drawer"
        open={settingVisible}
