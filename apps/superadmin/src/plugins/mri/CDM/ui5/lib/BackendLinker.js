sap.ui.define([
	...
	BackendLinker._postJson = function (oSettings,
		...
		return ConfigUtils.ajax(jQuery.extend({
			url: "/sap/hc/hph/cdw/config/services/config.xsjs",
			type: "POST",
			dataType: "json",
	...
	BackendLinker.testConfig = function (beConfig, path, exprToUse, useRefText,
		...
		BackendLinker
			._postJson(
				{
					url: "/sap/hc/hph/cdw/services/cdw_services.xsjs?action=attribute_infos_service",
					data: JSON.stringify({
						attributePath: path,
						exprToUse: exprToUse,
		...
		BackendLinker
			._postJson(
				{
					url: "/sap/hc/hph/cdw/services/cdw_services.xsjs?action=domain_values_service",
					data: JSON.stringify({
						attributePath: path,
						suggestionLimit: 10,
						exprToUse: exprToUse,
	...
	BackendLinker.getAttributeTypes = function (callback) {
		return BackendLinker
			._postJson(
				{
					url: "/sap/hc/hph/cdw/services/cdw_services.xsjs?action=attributeType_service",
					data: JSON.stringify({})
				}, callback);
	};
	...
	BackendLinker.filterSuggestion = function (data, callback) {
		...
		return BackendLinker
			._postJson(
				{
					url: "/sap/hc/hph/cdw/services/cdw_services.xsjs?action=table_suggestion_service",
					data: JSON.stringify({
	...
	BackendLinker.columnFilterSuggestion = function (data, callback) {
		return BackendLinker
			._postJson(
				{
					url: "/sap/hc/hph/cdw/services/cdw_services.xsjs?action=column_suggestion_service",
					data: JSON.stringify({
						table: data.table,
						mapping: data.mapping
					})
