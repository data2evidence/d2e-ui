sap.ui.define(
  ...
  function (jQuery, Button, Dialog, Text, TextArea, Link, VBox, IconPool) {
     ...
     *
     * Downport of the latest SAPUI5 MessageBox to fix issues with message details.
     *
     * @namespace
     * @alias sap.hc.hph.core.ui.MessageBox
     */
    var MessageBox = {};
    /**
    ...
    (function () {
      ...
      MessageBox.show = function (vMessage, mOptions) {
        ...
        if (
          typeof mOptions.actions !== "undefined" &&
          !jQuery.isArray(mOptions.actions)
        ) {
        ...
        function getInformationLayout(mOptions, oMessageText) {
          ...
          return new VBox({
            items: [oMessageText, oLink, oTextArea],
          });
        }
        function onclose() {
        ...
        function getInitialFocusControl() {
          ...
          if (mOptions.initialFocus) {
            if (mOptions.initialFocus instanceof sap.ui.core.Control) {
              // covers sap.m.Control cases
              oInitialFocusControl = mOptions.initialFocus;
            }
            if (typeof mOptions.initialFocus === "string") {
              // covers string and MessageBox.Action cases
              for (i = 0; i < aButtons.length; i++) {
                if (MessageBox.Action.hasOwnProperty(mOptions.initialFocus)) {
                  if (
                    that._rb
                      .getText("MSGBOX_" + mOptions.initialFocus)
                      .toLowerCase() === aButtons[i].getText().toLowerCase()
                  ) {
                  ...
                  if (
                    ...
                    aButtons[i].getText().toLowerCase()
                  ) {
                    oInitialFocusControl = aButtons[i];
                    break;
                  }
        ...
        if (typeof vMessage === "string") {
          vMessageContent = new Text({ textDirection: mOptions.textDirection })
            .setText(vMessage)
        ...
        if (
          mOptions &&
          mOptions.hasOwnProperty("details") &&
          mOptions.details !== ""
        ) {
