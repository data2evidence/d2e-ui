...
sap.ui.define(['jquery.sap.global', 'sap/ui/core/Control', './TimePickerSlidersRenderer', './TimePickerSlider'],
	function (jQuery, Control, SlidersRenderer, TimePickerSlider) {
		 ...
		 */
		var TimePickerSliders = Control.extend("sap.hc.mri.pa.ui.lib.datetimepicker.TimePickerSliders", /** @lends sap.m.TimePicker.prototype */ {
			metadata : {
				properties : {
				...
				aggregations: {
					 ...
					 */
					_columns: { type: "sap.hc.mri.pa.ui.lib.datetimepicker.TimePickerSlider", multiple: true, visibility: "hidden" }
				},
				associations: {
					 ...
					 */
					invokedBy: { type: "sap.hc.mri.pa.ui.lib.datetimepicker.TimePicker", multiple: false }
				}
			},
		...
		TimePickerSliders.prototype._setupLists = function (sFormat) {
			...
			if (sFormat.indexOf("a") !== -1) {
				this.addAggregation("_columns", new TimePickerSlider(this.getId() + "-listFormat", {
					...
					expanded: jQuery.proxy(onSliderExpanded, this),
					isCyclic: false
				}).addStyleClass("sapMriTimePickerSliderShort"));
			}
