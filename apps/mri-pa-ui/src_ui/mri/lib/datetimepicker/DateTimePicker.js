...
sap.ui.define(['jquery.sap.global', "sap/hc/mri/pa/ui/Utils", './DatePicker', './TimePickerSliders', 'sap/ui/model/type/Date'],
		function(jQuery, Utils, DatePicker, TimePickerSliders, Date1) {
	"use strict";
	 ...
	 */
	var DateTimePicker = DatePicker.extend("sap.hc.mri.pa.ui.lib.datetimepicker.DateTimePicker", /** @lends sap.m.DateTimePicker.prototype */ { metadata : {
	...
	var PopupContent = sap.ui.core.Control.extend("DateTimePickerPopup", {
		...
		renderer: function(oRm, oPopup) {
			...
			oRm.addClass("sapMriDateTimePopupCont");
			oRm.addClass("sapMriTimePickerDropDown");
			oRm.writeClasses();
			oRm.write(">");
			...
			if (oSwitcher && oSwitcher.getVisible()) {
				oRm.write("<div");
				oRm.addClass("sapMriTimePickerSwitch");
				oRm.writeClasses();
				oRm.write(">");
			...
			oRm.write("<div");
			oRm.addClass("sapMriTimePickerSep");
			oRm.writeClasses();
			oRm.write(">");
			oRm.write("</div>");
	...
	DateTimePicker.prototype._getPlaceholderPattern = function(oLocaleData, sPlaceholder) {
		...
		if (iSlashIndex > 0) {
			return oLocaleData.getDatePattern(sPlaceholder.substr(0, iSlashIndex)) + " " + oLocaleData.getTimePattern(sPlaceholder.substr(iSlashIndex + 1));
		} else {
			return oLocaleData.getDatePattern(sPlaceholder) + " " + oLocaleData.getTimePattern(sPlaceholder);
		}
	...
	DateTimePicker.prototype._createPopup = function(){
		...
		if (!this._oPopup) {
			var sOKButtonText = Utils.getText("MRI_PA_BUTTON_OK");
			var sCancelButtonText = Utils.getText("MRI_PA_BUTTON_CANCEL");
			...
			this._oPopup.addStyleClass("sapMriDateTimePopup");
