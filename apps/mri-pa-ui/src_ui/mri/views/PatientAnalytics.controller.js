...
], function (jQuery, Utils, AttributeMenuButton, BinningButton, BookmarkUtils, Filter, LazyMenu, MriFrontendConfig, PdfExport,
    BMv2Parser, AnnotateBM, ChartConfigService, ChartableCardsVisitor, ControlGenerator, ifr2backendifr, ifr2bookmark, KeyCounter,
    UrlFilterParser, FlexBox, Label, MessageBox, Popover, PositionContainer, CommonsLabel, ToggleButton, HorizontalLayout, VerticalLayout,
    JSONModel, Menu, MenuItem
) {
        "use strict";
         ...
         */
        sap.ui.controller("sap.hc.mri.pa.ui.views.PatientAnalytics", {
            /**
            ...
            onInit: function () {
                ...
                this._updateFilterDescriptionColumnWidthOnSplitterResize(this._patientFilter);
                 ...
                 */
                var oLocations = { // TODO: rename variable to oLocationsData
                    attr: []
                };
                var oSelectionsData = { // TODO: rename variable to oSelectionData
                    attr: []
                };
                ...
                this.sortMenu.addItem(new MenuItem({ text: "{i18n>MRI_PA_CHART_SORT_DEFAULT}" }));
                this.sortMenu.addItem(new MenuItem({ text: "{i18n>MRI_PA_CHART_SORT_REVERSE}" }));
                this.sortMenu.addItem(new MenuItem({ text: "{i18n>MRI_PA_CHART_SORT_ASCENDING}" }));
                this.sortMenu.addItem(new MenuItem({ text: "{i18n>MRI_PA_CHART_SORT_DESCENDING}" }));

                this.sortLabel = new CommonsLabel({
                    text: "{i18n>MRI_PA_CHART_SORT_LABEL}",
                    visible: "{" + Utils.models.LOCATIONS + ">/attr/" + sap.hc.mri.pa.ui.lib.Dimensions.Sort + "/visible}"
                });

                // Create selection controls
                this.binningButtons = [];
                var oLayoutAttributesContainer = this.getView().byId("layAttributesContainer");

                for (i = 0; i < sap.hc.mri.pa.ui.lib.Dimensions.Count; i++) {
                    var axisBtn = new AttributeMenuButton({
                        chartableFilterCards: "{" + Utils.models.SELECTIONS + ">/chartableFilterCards}",
                        enabled: "{" + Utils.models.LOCATIONS + ">/attr/" + i + "/enabled}",
                        selection: "{" + Utils.models.SELECTIONS + ">/attr/" + i + "/selection}",
                        visible: "{" + Utils.models.LOCATIONS + ">/attr/" + i + "/visible}",
                        width: "120px",
                        menu: (i === sap.hc.mri.pa.ui.lib.Dimensions.Sort) ? this.sortMenu : new LazyMenu({
                    ...
                    var oBinningButton = new BinningButton({
                        attribute: "{" + Utils.models.SELECTIONS + ">/attr/" + i + "/selection}",
                        binsize: "{" + Utils.models.SELECTIONS + ">/attr/" + i + "/binsize}",
                ...
                if (parentAttributeButton) {
                    ...
                    parentAttributeButton._updateTexts = function () {
                        var sortData = this.getModel(Utils.models.SELECTIONS).getData().attr[sap.hc.mri.pa.ui.lib.Dimensions.Sort].sortData;
                        if (sortData === "MRI_PA_CHART_SORT_ASCENDING" || sortData === "MRI_PA_CHART_SORT_DESCENDING" || sortData === "MRI_PA_CHART_SORT_REVERSE") {
                ...
                this.getView().setModel(new JSONModel(oLocations), Utils.models.LOCATIONS);
                this.getView().setModel(new JSONModel(oSelectionsData), Utils.models.SELECTIONS);
                ...
                this._labelTotal = new Label({
                    text: {
                        ...
                        formatter: function (oVal) {
                            ...
                            return Utils.getText("MRI_PA_PATIENT_COUNT_TOOLTIP_TOTAL") + ": " + oVal;
                        }
                    }
                });
                this._labelSelected = new Label({
                    text: {
                        ...
                        formatter: function (oVal) {
                            ...
                            return Utils.getText("MRI_PA_PATIENT_COUNT_TOOLTIP_MATCH") + ": " + oVal;
                        }
                    }
                });
            ...
            _updateFilterDescriptionColumnWidthOnSplitterResize: function (oFilter) {
                ...
                oSplitter.attachResize(function (oControlEvent) {
                    var iMaxWidth = Math.floor(oControlEvent.getParameter("newSizes")[0] * 0.4);
            ...
            _loadDefaultFilters: function () {
                ControlGenerator.generate(this._config.getInitialIFR(), this._patientFilter);
            ...
            onResetFilters: function () {
                if (!this._resetFiltersDialog) {
                    this._resetFiltersDialog = sap.ui.xmlfragment("resetFiltersDialog", "sap.hc.mri.pa.ui.views.ResetFiltersDialog", this);
                    this._resetFiltersDialog.addStyleClass(Utils.getContentDensityClass());                   
            ...
            onFilterChange: function (oEvent) {
                ...
                oModel.getProperty("/attr").forEach(function (mSelection, iIndex) {
                    var bValidSelection = aChartableFilterCards.some(function (mFilterCardInfo) {
                        return mFilterCardInfo.aAttributes.some(function (mAttributeInfo) {
                            return mAttributeInfo.bAvailable && mAttributeInfo.sAttributeInstance === mSelection.selection;
                        });
                    });
                    if (!bValidSelection) {
                        oModel.setProperty("/attr/" + iIndex + "/selection", sap.hc.mri.pa.ui.lib.Selection.Invalid);
                    }
                });
            ...
            onAxisSelect: function (oEvent) {
                ...
                var oBindingContext = oEvent.getSource().getBindingContext(Utils.models.SELECTIONS);
                oBindingContext.getModel().setProperty(oBindingContext.getPath() + "/selection", sSelection);
